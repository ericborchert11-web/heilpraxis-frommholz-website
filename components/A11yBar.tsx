...
export function A11yBar({ lang }: { lang: Locale }) {
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-[200] flex items-center justify-between bg-anthracite px-6 py-2 text-cream text-xs border-b border-gold">
      {/* Unter sm bleibt nur das Icon — die Nummer steht ohnehin im Hero, im
          Kontaktbereich und im Footer. `aria-label` trägt den zugänglichen
          Namen, weil die sichtbare Beschriftung dann fehlt. */}
      <a
        href={`tel:${BUSINESS.phone}`}
        aria-label={BUSINESS.phoneDisplay}
        // `self-stretch` + `px-2 -mx-2`: vergrößert die Tippfläche auf die
        // volle Zeilenhöhe, ohne Breite oder Höhe der Leiste zu verändern.
        className="flex items-center gap-2.5 self-stretch px-2 -mx-2 font-normal tracking-wide hover:text-gold-soft"
      >
